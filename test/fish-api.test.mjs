...
describe("Aquário — API fish", () => {
  ...
  const bridgeOpts = { activityLogPath: path.join(os.tmpdir(), `ma-act-${process.pid}-${Date.now()}.json`), taskBoardPath: path.join(os.tmpdir(), `ma-tb-${process.pid}-${Date.now()}.json`) };
