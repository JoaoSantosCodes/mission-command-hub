...
import { getAgentStatusesFromLogs, type AgentLiveStatus } from '@/lib/agent-status-from-logs';
import {
...
function AgentCharAvatar({ agentId, index }: { agentId: string; index: number }) {
  ...
  const charIndex =
    typeof profile.avatarIndex === 'number' ? profile.avatarIndex : charIndexForAgentId(agentId);
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      try {
        ...
        ctx.drawImage(img, frameX, frameY, FRAME_W, FACE_H, pad + offX, pad + offY, target, target);
      } catch {
        setFailed(true);
      }
    };
...
export function TeamStatusOverview({
  ...
  return (
    <div className="team-status-panel flex h-full flex-col rounded-xl border border-border bg-card/50">
      <div className={`border-b border-border ${compact ? 'px-2 py-1.5' : 'px-3 py-2'}`}>
        <h2 className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
          Estado da equipa
        </h2>
        <p className="mt-0.5 text-[10px] tracking-wide text-emerald-400/80">
          Feed · Architecture Agents Hub
        </p>
        ...
        <div
          className={`${compact ? 'mt-1.5' : 'mt-2'} flex gap-2`}
          role="tablist"
