...
if (isProd && process.env.TRUST_PROXY !== '1') {
  warn(
    'Produção: TRUST_PROXY não é 1 — atrás de reverse proxy, rate limit por IP pode estar incorrecto.'
  );
}
...
if (
  ...
  !process.env.SLACK_WEBHOOK_URL.includes('hooks.slack.com')
) {
...
if (process.env.MISSION_DOUBTS_LLM === '1' && key.length < 8) {
  warn(
    'MISSION_DOUBTS_LLM=1 mas chave LLM em falta ou curta — painel Dúvidas fica sem modelo no servidor.'
  );
}
