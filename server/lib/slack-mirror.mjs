...
const SLACK_TIMEOUT_MS = Math.min(Math.max(Number(process.env.SLACK_WEBHOOK_TIMEOUT_MS) || 8_000, 2_000), 20_000);
...
export async function mirrorActivityToSlack(entry) {
  ...
  const action = String(entry.action ?? "").trim().slice(0, 500);
